...
With all of this, we can understand a little better why the matching algorithm works. If a regular expression is represented by its derivatives and we iteratively apply the derivative to $R$ (w.r.t. the current character we are looking at in the string $s$), then we should be construcing a descending chain of subsets ($S_1 \subseteq S2 \subseteq S3 \subseteq ... $) of strings in our alphabet $\Sigma$, which should end in an empty set **exactly** when we run out of characters in $s$. If the string matches $R$, then each successive derivative $d_i$ "approximates" $R$ in the sense that the acceptance set $S_{d_i}$ is a subset of the acceptance set of $R$.
